...
namespace Microsoft.Legal.MatterCenter.Utility
{
    ...
    public class MailMessageParser : MailMessage
    {
        ...
        public void SetContentDisposition(string headerLineContent)
        {
            ...
            {
                string[] mailParameters = headerLineContent.Split(new string[] { ConstantStrings.Semicolon }, StringSplitOptions.RemoveEmptyEntries);
                ...
                {
                    string[] nameValue = mailParameters[mailParametersCount].Split(new string[] { ConstantStrings.OperatorEqual }, StringSplitOptions.None);
                    ...
                    value = value.Replace(ConstantStrings.DoubleQuote, string.Empty);

                    switch (name)
                    {
                        case ConstantStrings.MailAttributes.FILENAME:
                            this.ContentDisposition.FileName = value;
                            break;
                        case ConstantStrings.MailAttributes.SIZE:
                            this.ContentDisposition.Size = long.Parse(value, CultureInfo.InvariantCulture);
                            break;
                        case ConstantStrings.MailAttributes.CREATION_DATE:
                            this.ContentDisposition.CreationDate = DateTime.Parse(value, CultureInfo.InvariantCulture);
                            break;
                        case ConstantStrings.MailAttributes.MODIFICATION_DATE:
                            this.ContentDisposition.ModificationDate = DateTime.Parse(value, CultureInfo.InvariantCulture);
                            break;
                        case ConstantStrings.MailAttributes.READ_DATE:
        ...
        public void SetContentTypeFields(string mailContentType)
        {
            if (!string.IsNullOrEmpty(mailContentType))
            {
                ...
                {
                    this.ContentType = new ContentType(ConstantStrings.MailContentType);
                }
                else
                {
                ...
                {
                    this.ContentType.MediaType = ConstantStrings.MailMediaType;
                }
                else
                {
                    string mediaTypeString = this.ContentType.MediaType.Trim();
                    int slashPosition = this.ContentType.MediaType.IndexOf(ConstantStrings.ForwardSlash, StringComparison.Ordinal);
                    if (1 > slashPosition)
                    {
                        this.MediaMainType = mediaTypeString;
                        if (string.Equals(this.MediaMainType, ConstantStrings.TextMediaMainType, StringComparison.OrdinalIgnoreCase))
                        {
                            this.MediaSubType = ConstantStrings.MailMediaSubType;
                        }
                        else
                        {
                ...
                this.IsBodyHtml = this.MediaSubType == ConstantStrings.HtmlMediaMainType;
            }
        }
