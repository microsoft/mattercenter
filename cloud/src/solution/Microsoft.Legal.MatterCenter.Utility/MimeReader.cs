...
namespace Microsoft.Legal.MatterCenter.Utility
{
    ...
    public class MailMimeReader : IDisposable
    {
        ...
        public static MailAddress ConvertToMailAddress(string address)
        {
            ...
            {
                address = address.Replace(ConstantStrings.DoubleQuote, string.Empty);
                address = address.Trim();
                result = new MailAddress(address);
                if (ConstantStrings.OpeningAngularBracket + ConstantStrings.ClosingAngularBracket == address)
                {
                    result = null;
                }
            }
        ...
        public MailMessageParser GetEmail(Stream emailStream)
        {
            ...
            if (messageMimeReturnCode == MimeEntityReturnCode.bodyComplete || messageMimeReturnCode == MimeEntityReturnCode.parentBoundaryEndFound)
            {
                ...
                {
                    string toField = message.Headers[ConstantStrings.MailMessageReceiverHeader];
                ...
                {
                    string mailFrom = message.Headers[ConstantStrings.MailMessageSenderHeader];
            ...
            try
            {
                if (!string.IsNullOrEmpty(date))
                {
                      ...
                      out returnDateTime))
                    {
                        ...
                        int dateLength = cleanDateTime.IndexOf(ConstantStrings.COLON, StringComparison.CurrentCulture) - 3;
        ...
        public DateTime ProcessReceivedDate(string headerValue)
        {
            ...
            {
                string datePortion = headerValue.Substring(headerValue.LastIndexOf(ConstantStrings.Semicolon, StringComparison.OrdinalIgnoreCase) + 1);
        ...
        protected bool ReadMultipleLine(out string response)
        {
            ...
            else if (0 < response.Length && ConstantStrings.Period == Convert.ToString(response[0], CultureInfo.InvariantCulture))
            {
                if (ConstantStrings.Period == response)
                {
                    result = false;
                }
                ////remove the first '.'
        ...
        private static void AddMailAddresses(string addresses, MailAddressCollection addressCollection)
        {
            ...
            Regex regexObj = new Regex(ConstantStrings.MaiAddressFieldregex);
            MatchCollection addressMatch = regexObj.Matches(addresses);
            foreach (Match match in addressMatch)
            {
                string quotedString = match.Value.Replace(ConstantStrings.Comma[0], (char)3);
                addresses = addresses.Replace(match.Value, quotedString);
            }

            string[] addressSplit = addresses.Split(ConstantStrings.Comma[0]);
            foreach (string address in addressSplit)
            {
                // be sure to add the comma back if it was replaced
                mailAddressField = ConvertToMailAddress(address.Replace((char)3, ConstantStrings.Comma[0]));
        ...
        private static TransferEncoding ConvertToTransferEncoding(string transferEncodingString)
        {
            ...
            {
                case ConstantStrings.MailAttributes.BIT_7:
                case ConstantStrings.MailAttributes.BIT_8:
                    result = TransferEncoding.SevenBit;
                    break;
                case ConstantStrings.MailAttributes.QUOTED_PRINTABLE:
                    result = TransferEncoding.QuotedPrintable;
                    break;
                case ConstantStrings.MailAttributes.BASE64:
        ...
        private static void AddChildPartsToParent(MailMessageParser child, MailMessageParser parent)
        {
            ...
            if (child.MediaMainType == ConstantStrings.TextMediaMainType && null != child.ContentStream && null != child.Parent.ContentType && child.Parent.ContentType.MediaType.ToUpperInvariant() == ConstantStrings.MultipartMediaType)
            {
        ...
        {
            bool hasParentBoundary = parentBoundaryStart.Length > 0;
            string parentBoundaryEnd = parentBoundaryStart + ConstantStrings.Hyphen + ConstantStrings.Hyphen;
            MimeEntityReturnCode boundaryMimeReturnCode;
            ...
            ////inherited as well
            message.SetContentTypeFields(ConstantStrings.MailContentType);
            ...
            while (true)
            {
                ...
                if (ConstantStrings.Space == Convert.ToString(response[0], CultureInfo.InvariantCulture) || ConstantStrings.HorizontalTab == Convert.ToString(response[0], CultureInfo.InvariantCulture))
                {
                    if (completeHeaderField == null)
                    {
                        while (this.ReadMultipleLine(out response))
                    ...
                    {
                        if (ConstantStrings.Space != Convert.ToString(completeHeaderField[completeHeaderField.Length - 1], CultureInfo.InvariantCulture))
                        {
                            completeHeaderField += ConstantStrings.Space + response.TrimStart(whiteSpaceChars);
                        }
                        else
                        {
            ...
            switch (message.ContentTransferEncoding)
            {
                ...
                case TransferEncoding.Base64:
                    ...
                    if (message.MediaMainType == ConstantStrings.TextMediaMainType)
                    {
            ...
            if (null != message.ContentDisposition && message.ContentDisposition.DispositionType.ToUpperInvariant() == ConstantStrings.MailAttributes.ATTACHMENT && !isAttachmentSaved)
            {
                SaveAttachment(message);
                isAttachmentSaved = true;
            }
        ...
        private void ProcessHeaderField(MailMessageParser message, string headerField)
        {
            ...
            int separatorPosition = headerField.IndexOf(ConstantStrings.COLON, StringComparison.CurrentCulture);
            if (0 < separatorPosition)
            {
                ...
                {
                    case ConstantStrings.MailAttributes.BCC:
                        AddMailAddresses(headerLineContent, message.Bcc);
                        break;
                    case ConstantStrings.MailAttributes.CC:
                        AddMailAddresses(headerLineContent, message.CC);
                        break;
                    case ConstantStrings.MailAttributes.CONTENT_DESCRIPTION:
                        message.ContentDescription = headerLineContent;
                        break;
                    case ConstantStrings.MailAttributes.CONTENT_DISPOSITION:
                        message.SetContentDisposition(headerLineContent);
                        break;
                    case ConstantStrings.MailAttributes.CONTENT_ID:
                        message.ContentId = headerLineContent;
                        break;
                    case ConstantStrings.MailAttributes.CONTENT_TRANSFER_ENCODING:
                        message.ContentTransferEncoding = ConvertToTransferEncoding(headerLineContent);
                        break;
                    case ConstantStrings.MailAttributes.CONTENT_TYPE:
                        message.SetContentTypeFields(headerLineContent);
                        break;
                    case ConstantStrings.MailAttributes.DATE:
                        message.DeliveryDate = this.ConvertToDateTime(headerLineContent);
                        break;
                    case ConstantStrings.MailAttributes.FROM:
                        ...
                        break;
                    case ConstantStrings.MailAttributes.SENDER:
                        message.Sender = ConvertToMailAddress(headerLineContent);
                        break;
                    case ConstantStrings.MailAttributes.SUBJECT:
                        message.Subject = headerLineContent;
                        break;
                    case ConstantStrings.MailAttributes.TO:
                        AddMailAddresses(headerLineContent, message.To);
                        break;
                    case ConstantStrings.MailAttributes.IMPORTANCE:
                        message.MailImportance = headerLineContent;
                        break;
                    case ConstantStrings.MailAttributes.CATEGORIES:
                        message.MailCategories = headerLineContent;
                        break;
                    case ConstantStrings.MailAttributes.RECEIVED:
