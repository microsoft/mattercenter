...
function GetContentTypeID(){
    ...
    Write-Host -f Green "Getting parent Content Type ID"
    ...
    try {
		...
		Write-Host -f Green "Parent Content Type ID retrieved"
    } catch [Exception]
    {
        $parentContentTypeId = ""
        write-host -f Red "Failed to get the parent Content Type ID"
...
function UpdateAppFile(){
	...
	try
	{
		...
		$package.Close()
        Write-Host -f Green "Updated the $fileName package"
	} catch [Exception]
    {
        $parentContentTypeId = ""
        Write-Host -f Red "Failed to update the $fileName package"
        Write-Log $ErrorLogFile $_.Exception.ToString()
    }
}
...
if (3 -eq $Configurations.Count){
    ...
    $ParentContentTypeId = GetContentTypeID -IsDeployedOnAzure $IsDeployedOnAzure -UserName $Username -Password $Password
